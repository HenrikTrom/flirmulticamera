...
int main(int argc, char **argv)
{
    ...
    std::vector<std::string> windowNames{settings.SNs.size()};
    std::vector<Frame> imgs{settings.SNs.size()};

    // When using window thread waitkey does not work
    //cv::startWindowThread();
    const int windowHeight = SCREEN_HEIGHT / settings.SNs.size();
    int windowWidth = settings.width;
    if (SCREEN_WIDTH - settings.width > SCREEN_WIDTH/3){
        windowWidth = SCREEN_WIDTH/3;
    }

    for (int cidx = 0; cidx<settings.SNs.size(); cidx++)
    {
        windowNames.at(cidx) = settings.SNs.at(cidx);
